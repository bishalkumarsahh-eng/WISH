...
def is_preview_valid(site, token):
    ...
    # preview link is generated. This gives the creator a full preview window.
...
async def site_audio(slug: str, token: str | None = Query(default=None), request: Request = None):
    """Serve the Premium song with browser-friendly media headers.

    The Telegram file is fetched once per request and returned inline. Range
    requests are accepted so mobile browsers and Telegram's in-app browser can
    seek/load the audio normally.
    """
    site = await authorized_site(slug, token)
    ...
    try:
        ...
        if unit.strip().lower() != "bytes":
            raise ValueError
        start_s, end_s = ranges.split(",", 1)[0].split("-", 1)
...
def render_premium_story_legacy(site, preview=False, preview_token=None):
    ...
    audio_url = f"/media/{site['slug']}/audio{token_q}"
    song_html = (f"<link rel='preload' as='audio' href='{audio_url}' crossorigin='anonymous'><audio id='wx-song' preload='auto' loop autoplay playsinline crossorigin='anonymous' src='{audio_url}'></audio>"
